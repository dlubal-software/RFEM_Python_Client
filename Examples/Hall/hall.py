...
if __name__ == '__main__':
  ...
  # Vertical Bracing
  def ask_user():
    check = str(raw_input("Do you want to include bracing ? (Y/N): ")).lower().strip()
    try:
        if check[0] == 'y':
            return 
              ...
              while i <= n-1:
                k = j + (i-1)*4
               Member(k+1, MemberType.TYPE_TENSION, (i-1)*5+1, (i-1)*5+7 , 0.0,  2, 2)
               Member(k+2, MemberType.TYPE_TENSION, (i-1)*5+2, (i-1)*5+6 , 0.0,  2, 2)
               Member(k+3, MemberType.TYPE_TENSION, (i-1)*5+5, (i-1)*5+9 , 0.0,  2, 2)
               Member(k+4, MemberType.TYPE_TENSION, (i-1)*5+4, (i-1)*5+10, 0.0,  2, 2)
               i += 1
        elif check[0] == 'n':
            return False
        else:
