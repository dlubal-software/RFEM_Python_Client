...
class LoadCasesAndCombinations():
    ...
    def getAvailableLoadActionCategoryTypes(model=Model):
        ...
        try:
            df = pd.read_csv(r'RFEM\LoadCasesAndCombinations\applicableLoadActionCategoryTypes.csv')
            df = df.transpose(copy=False)

        except:
            print('Function is based on the assumption that the current working directory is at the root directory of RFEM_PYTHON_CLIENT.')

        return ast.literal_eval(df.loc[str(standardCode)][0])
