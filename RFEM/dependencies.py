...
try:
    ...
    if instSUDS == 'y':
        ...
        try:
            ...
            subprocess.call('python -m pip install requests six suds-py3 xmltodict pytest mock psutil --user')
        except:
            print('WARNING: Installation of modules failed!')
            print('Please use command "python -m pip install requests six suds-py3 xmltodict pytest mock psutil --user" in your Command Prompt.')
            input('Press Enter to exit...')
            sys.exit()
    else:
        input('Press Enter to exit...')
